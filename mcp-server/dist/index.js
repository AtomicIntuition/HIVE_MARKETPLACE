...
server.tool("search-tools", "Search and browse MCP tools on Hive Market. Filter by query, category, pricing, and sort order.", searchToolsSchema.shape, async ({ query, category, pricing, sort, limit }) => {
...
server.tool("get-tool", "Get full details about a specific MCP tool including description, features, pricing, and install info.", getToolSchema.shape, async ({ slug }) => {
...
server.tool("get-tool-config", "Get the MCP configuration JSON for a tool, ready to paste into your client config file.", getToolConfigSchema.shape, async ({ slug, client }) => {
...
server.tool("list-categories", "List all tool categories with descriptions and tool counts.", {}, async () => {
...
server.tool("list-stacks", "List all curated tool stacks (pre-built combinations of MCP tools for common use cases).", {}, async () => {
...
server.tool("get-stack", "Get full details about a stack including all tools and combined MCP config.", getStackSchema.shape, async ({ slug }) => {
    const result = await getStackHandler({ slug });
    if (!result) {
        return {
            content: [
                { type: "text", text: `Stack "${slug}" not found.` },
            ],
            isError: true,
        };
    }
...
server.tool("recommend-tools", "Get personalized tool recommendations based on a use case description.", recommendToolsSchema.shape, async ({ useCase, maxResults }) => {
